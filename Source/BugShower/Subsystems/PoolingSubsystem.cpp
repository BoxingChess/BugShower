...
void UPoolingSubsystem::RegisterPoolForClass(TSubclassOf<AActor> ActorClass, int32 Size)
{
	...
	UE_LOG(LogTemp, Warning, TEXT("[PoolingSubsystem] ?윟 RegisterPoolForClass: Creating pool for %s with size %d"), *ActorClass->GetName(), Size);
	...
	for (int32 i = 0; i < Size; i++)
	{
		...
		if (SpawnedActor && SpawnedActor->Implements<USpawnable>())
		{
			...
			if (i < 10)
			{
				UE_LOG(LogTemp, Warning, TEXT("[PoolingSubsystem] ?윟 Added Actor %p to pool (index %d)"), SpawnedActor, i);
			}
	...
	UE_LOG(LogTemp, Warning, TEXT("[PoolingSubsystem] ?윟 Pool registered: All.Num()=%d, Available.Num()=%d"), NewPool.All.Num(), NewPool.Available.Num());
...
void UPoolingSubsystem::ReturnToPoolByClass(TScriptInterface<ISpawnable> Object)
{
	...
	UE_LOG(LogTemp, Warning, TEXT("[PoolingSubsystem] ?윝 ReturnToPoolByClass called for Actor: %p"), Actor);
	...
	UE_LOG(LogTemp, Warning, TEXT("[PoolingSubsystem] ?윝 Available.Num() BEFORE Add: %d"), PoolData->Available.Num());
	...
	UE_LOG(LogTemp, Warning, TEXT("[PoolingSubsystem] ?윝 Available.Num() AFTER Add: %d"), PoolData->Available.Num());
